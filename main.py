...
#
#         # "email": "c@gmail.com",
#         "hashed_password": "$2b$12$7vGtC6lJ0Dnrw/RMBQJgROyXlWJ1N8z7T3kWsWqcPFMWTq1.AcoH2",
#     }
# }
...
@app.post('/revenue')
def create(info:Request, req:schema.Access, db:Session=Depends(get_db)):
    # items_list: await info.json()
    # filter_date=datetime.strptime(str(item['date']),"%y-%m_%d")
    ...
    for x in reven:
        response['data'].append({'sno':x[0], 'Location':x[1], 'Target':x[2],'Achieved':x[3],'Achieved_percentage':x[4], 'clustername':x[5]})
